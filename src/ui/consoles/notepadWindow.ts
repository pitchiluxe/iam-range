...
export function renderNotepadWindow(body: HTMLElement): void {
  body.style.cssText =
    'display:flex;flex-direction:column;height:100%;background:var(--panel);font-family:Consolas,Monaco,"Courier New",monospace;';
  ...
  lineNums.style.cssText = `
    flex-shrink:0;width:40px;background:var(--bg);border-right:1px solid var(--border);
    color:var(--muted);font-size:12px;line-height:1.6;text-align:right;
  ...
  ta.style.cssText = `
    ...
    color:var(--fg);font-family:Consolas,Monaco,"Courier New",monospace;
  ...
  statusBar.style.cssText = `
    flex-shrink:0;height:26px;background:var(--panel-alt);border-top:1px solid var(--border);
  ...
  statusBar.innerHTML = `
    ...
    <span id="np-autosave" style="margin-left:auto;color:var(--accent);opacity:0;">Saved</span>
  `;
